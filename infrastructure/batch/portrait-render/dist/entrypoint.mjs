...
function detectionsToCropKeyframes(output) {
  return output.positions.filter((p) => p.x >= 0 && p.source !== "none").map((p) => ({
    time: p.time,
    x: ballXToCropX(p.x),
    source: p.source === "ball" ? "ai_ball" : p.source === "tracked" ? "ai_tracked" : "ai_cluster",
...
function filterOutliers(keyframes) {
  ...
  return keyframes.filter((kf, i) => {
    ...
    if (Math.abs(kf.x - prev.x) >= TELEPORT_PX && Math.abs(kf.x - next.x) >= TELEPORT_PX && Math.abs(next.x - prev.x) < TELEPORT_PX) {
      return false;
    }
    if (kf.confidence >= OUTLIER_CONF_THRESHOLD) return true;
    const expectedX = prev.x + (next.x - prev.x) * (kf.time - prev.time) / (next.time - prev.time || 1);
    ...
    if (prevBall && nextBall) {
      ...
      if (dt > 0) {
        const ballExpectedX = prevBall.x + (nextBall.x - prevBall.x) * (kf.time - prevBall.time) / dt;
    ...
    if (j - i + 1 >= DISTRACTOR_MIN_FRAMES) {
      ...
      for (let r = i; r <= j; r++) {
        ...
        if (before && after) {
          ...
          expected = ax(before) + (ax(after) - ax(before)) * ((k.time - before.time) / span);
...
function preserveHighVelocity(original, simplified, minGapSeconds = 2, minVelocity = 150) {
...
function filterZigzags(keyframes, sceneCuts = [], zigzagThreshold = DEFAULTS.ZIGZAG_THRESHOLD) {
  ...
  while (changed) {
    ...
    for (let i = 1; i < result.length - 1; i++) {
      ...
      const isReversal = d1 > 0 && d2 < d1 && d1 - d2 > zigzagThreshold || d1 < 0 && d2 > d1 && d2 - d1 > zigzagThreshold;
      if (isReversal) {
        changed = true;
      } else {
        next.push(curr);
...
function removeNearDuplicates(keyframes) {
  ...
  for (let i = 1; i < keyframes.length; i++) {
    ...
    const dt = curr.time - prev.time;
    const dx = Math.abs(curr.x - prev.x);
    const isStaleCluster = curr.source === "ai_cluster" && dt < 2 && dx < P.NEAR_DUPLICATE_PX;
    if (dt < P.NEAR_DUPLICATE_TIME && dx < P.NEAR_DUPLICATE_PX || isStaleCluster) {
      if (isStaleCluster) {
...
function insertHoldBeforePan(keyframes) {
  ...
  for (let i = 1; i < keyframes.length; i++) {
    ...
    const prevVelocity = prevPrev ? Math.abs(prev.x - prevPrev.x) / (prev.time - prevPrev.time || 1) : 0;
...
function filterDeadZone(keyframes) {
  ...
  for (let i = 1; i < keyframes.length; i++) {
    ...
    if (dx < P.DEAD_ZONE_PX && (prev.source === "ai_cluster" && curr.source === "ai_cluster" || prev.source === "ai_tracked" && curr.source === "ai_tracked" && dt < 1.5)) {
      continue;
    }
    if (prev.source === "ai_ball" && curr.source === "ai_ball" && dt < 0.8 && dx < 120) {
...
function filterEdgeDwell(keyframes) {
  ...
  if (edgeRunStart >= 0) {
    const runDuration = keyframes[keyframes.length - 1].time - keyframes[edgeRunStart].time;
...
function filterTrackedDrift(keyframes, original) {
  ...
  for (let i = 1; i < keyframes.length - 1; i++) {
    ...
    if ((isExtremeLow(kf.x) || isExtremeHigh(kf.x)) && isCentral(prev.x) && isCentral(next.x)) {
      if (next.time - prev.time > MAX_GAP) {
        const midTime = prev.time + (next.time - prev.time) / 2;
        let best = null;
        let bestDist = Infinity;
        for (const okf of original) {
          if (okf.time > prev.time + 0.5 && okf.time < next.time - 0.5 && isNonExtreme(okf.x)) {
    ...
    for (let i = 1; i < result.length; i++) {
      ...
      if (gap >= MAX_GAP) {
        ...
        for (const kf of original) {
          if (kf.time > prev.time + 0.5 && kf.time < curr.time - 0.5 && !isExtreme(kf.x)) {
...
function simplifyCropKeyframes(keyframes, sceneChanges, overrides) {
  ...
  if (final.length >= 2) {
    ...
    if (last.source !== "user" && (last.x <= 50 || last.x >= MAX_CROP - 50) && prev.x > 50 && prev.x < MAX_CROP - 50) {
      final.pop();
    }
  }
...
function autoKeyframesFromDetection(detection) {
  ...
  return {
    ...
    quality: {
      ...
      mean_conf: conf.length ? conf.reduce((a, p) => a + (p.conf ?? 0), 0) / conf.length : 0,
      duration_seconds: positions.length ? Math.max(...positions.map((p) => p.time)) : 0,
...
async function getDetection(eventId, clip) {
  const { data: cached } = await supabase.from("playhub_crop_detections").select("detection").eq("veo_highlight_id", eventId).maybeSingle();
  ...
  const blobOk = Array.isArray(detection?.positions) && Array.isArray(detection?.scene_changes) && JSON.stringify(detection).length <= 2e6;
  if (blobOk) {
...
async function writeRenderGuarded(row) {
  ...
  const { count, error: upErr } = await supabase.from("playhub_portrait_renders").update(
    {
      status: row.status,
      error: row.error,
    ...
    { count: "exact" }
  ).eq("provider_event_id", row.provider_event_id).in("status", ["draft", "error"]);
  if (upErr) throw new Error(`renders update: ${upErr.message}`);
  if (count && count > 0) return true;
  const { error: insErr } = await supabase.from("playhub_portrait_renders").insert({ ...row, updated_at: now });
...
async function main() {
  const { data: events, error: evErr } = await supabase.from("playhub_recording_events").select("id, provider_event_id, timestamp_seconds").eq("provider", "veo").eq("event_type", "goal").eq("provider_recording_id", MATCH_SLUG);
  if (evErr) throw new Error(`events query: ${evErr.message}`);
  if (!events?.length) {
    console.log("no goal events for match \u2014 nothing to do");
    return;
  }
  const { data: cacheRow, error: cacheErr } = await supabase.from("playhub_veo_match_content_cache").select("highlights").eq("match_slug", MATCH_SLUG).maybeSingle();
  ...
  const { data: existing } = await supabase.from("playhub_portrait_renders").select("provider_event_id, status, attempts").eq("provider_recording_id", MATCH_SLUG);
  ...
  for (const event of events) {
    ...
    if (status === "error" && (prior?.attempts ?? 0) >= MAX_ATTEMPTS && !RERENDER) {
      skipped++;
      continue;
    }
    ...
    try {
      ...
      const { keyframes, sceneChanges, quality } = autoKeyframesFromDetection(detection);
      ...
      const { error: upErr } = await supabase.storage.from(STORAGE_BUCKET).upload(base.storage_path, rendered, {
