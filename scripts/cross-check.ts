...
;(async()=>{
  ...
  console.log(`Team slugs in listClubsAndTeams output (${lyl.teams.length} teams):`)
  for (const t of lyl.teams) console.log(`  id=${t.id.slice(0,8)} slug=${t.slug.padEnd(35)} name="${t.name}"`)
  ...
  for (const r of recs) {
    if (!r.team) { countByTeam.set('(no team)', (countByTeam.get('(no team)') ?? 0) + 1); continue }
    countByTeam.set(r.team, (countByTeam.get(r.team) ?? 0) + 1)
  }
  console.log(`\nRecording counts by team UUID (resolved through teamSlugById):`)
  for (const [uuid, count] of [...countByTeam.entries()].sort((a,b) => b[1] - a[1])) {
    const slug = uuid === '(no team)' ? uuid : teamSlugById.get(uuid) ?? `??UNKNOWN:${uuid.slice(0,12)}`
