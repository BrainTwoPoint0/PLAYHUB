...
T0,T1=30.0,58.0; m=(rt>=T0)&(rt<=T1); idx=np.where(m)[0]
pan=sm(np.array([u2pt(rpx[i],rpy[i])[0] for i in idx])); tilt=sm(np.array([u2pt(rpx[i],rpy[i])[1] for i in idx]))
fov=sm(np.clip(fw[idx]*95,20,46)); fovflat=np.clip(fov*FOVMUL,20,58); tt=rt[idx]
capr=cv2.VideoCapture(RAW); capp=cv2.VideoCapture(PLAY); fps=capp.get(5) or 25
vw=cv2.VideoWriter("/tmp/imitation/final_follow.mp4",cv2.VideoWriter_fourcc(*"mp4v"),fps,(W*2,H))
...
for fi in range(int(T0*fps),int(T1*fps)):
    ...
    fv=clamp_fov(pn,tl,fv)
    a=lab(render(rf,pn,tl,fv),"PLAYHUB (our flatten, wide framing 1.35x)",(0,255,255))
    b=lab(cv2.resize(pf,(W,H)) if okp else np.zeros((H,W,3),np.uint8),"SPIIDEO Play",(210,210,210))
    fr=np.hstack([a,b]); vw.write(fr); n+=1
    if fi==int(47*fps): cv2.imwrite("/tmp/imitation/final_follow_still.png",fr)
capr.release(); capp.release(); vw.release()
print(f"wrote /tmp/imitation/final_follow.mp4 ({n} frames) + still")
