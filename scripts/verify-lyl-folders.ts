...
async function main(): Promise<void> {
  const { listRecordings, listClubsAndTeams } =
  ...
  const [listRes, teamsRes, assignmentsRes] = await Promise.all([
    ...
    supabase
      ...
      .select(
        'recording_slug, recording_title, home_team_uuid, away_team_uuid, away_accepted_recording_uuid, status'
      )
  ...
  if (!teamsRes.success || !teamsRes.data)
    throw new Error(`listClubsAndTeams: ${teamsRes.message}`)
  ...
  for (const team of teamNames) {
    ...
    console.log(
      `  ${team.padEnd(28)} ${String(items.length).padStart(3)} total  (${originals} orig + ${shares} share)`
    )
  }
  ...
  for (const a of assignments) {
    ...
    if (!r) {
      mismatches.push(
        `MISSING: ${shareSlug} (assignment exists but no recording in Veo)`
      )
      continue
    }
    ...
    if (!expectedName) {
      mismatches.push(
        `UNKNOWN TEAM: ${shareSlug} → away_team_uuid=${a.away_team_uuid} not in Veo team list`
      )
      continue
    }
    if (r.team !== expectedName) {
      mismatches.push(
        `MISPLACED: ${a.recording_title}\n           slug=${shareSlug}\n           current="${r.team ?? '<none>'}" expected="${expectedName}"`
      )
    }
  }
  ...
  if (mismatches.length === 0) {
    console.log(
      `  ✓ ALL ${assignments.length} share-copies are in the correct away-team folder.\n`
    )
  } else {
