...
;(async () => {
  const { getVeoSession, shutdownVeoSession } = await import('../src/lib/veo/auth')
  ...
  for (const url of [
    ...
    console.log(`body (first 800 chars): ${typeof res.body === 'string' ? res.body.slice(0, 800) : JSON.stringify(res.body).slice(0, 800)}`)
    console.log()
  }
