...
;(async () => {
  ...
  const lyl = teamsRes.data!.clubs.find(
    (c) => c.slug === 'london-youth-league'
  )!
  ...
  const byTitle = new Map<
    string,
    { recording_slug: string; team_slug: string }[]
  >()
  for (const r of recs) {
    const title = (r.title || '(untitled)').trim()
    const teamSlug = r.team
      ? (teamById.get(r.team)?.slug ?? `unknown:${r.team.slice(0, 8)}`)
      : '(no team)'
  ...
  for (const [title, items] of sorted) {
    ...
    console.log(
      `  ${marker} ${items.length}× ${title.slice(0, 55).padEnd(55)} → ${teamSlugs.join(', ')}`
    )
  }
