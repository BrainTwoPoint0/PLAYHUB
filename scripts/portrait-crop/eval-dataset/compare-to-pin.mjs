...
const pinPath =
  pinIdx >= 0 ? args[pinIdx + 1] : path.join(RESULTS_DIR, 'pin.json')
...
function latestResults() {
  const files = fs
    ...
    .sort((a, b) => b.t - a.t)
  if (!files.length)
    throw new Error('No results/<sha>.json runs found — run eval.ts first.')
...
console.log(`\nCompare-to-pin`)
console.log(
  `  pin:     ${path.basename(pinPath)} (sha ${pin.pinned_sha}, ${pin.pinned_at})`
)
console.log(
  `  run:     ${path.basename(resultsPath)} (sha ${run.git_sha ?? '?'})`
)
console.log(
  `  gate:    >${(THRESHOLD * 100).toFixed(0)}% regression on frozen_holdout clips`
)
...
for (const clipId of pin.clip_ids ?? []) {
  ...
  for (const { key, higherBetter } of GATED) {
    ...
    const fmt = (v) =>
      Math.abs(v) < 1 ? v.toFixed(3) : Math.round(v).toString()
    console.log(
      `      ${key.padEnd(20)} ${fmt(o).padStart(8)} → ${fmt(n).padStart(8)}  ${pct}%  ${tag}`
    )
...
if (regressions.length === 0 && missing.length === 0) {
  console.log(
    `  ✅ PASS — no regressions on ${(pin.clip_ids ?? []).length} frozen_holdout clips.`
  )
  process.exit(0)
}
if (regressions.length) {
  console.log(
    `  ❌ FAIL — ${regressions.length} regression(s) >${(THRESHOLD * 100).toFixed(0)}%:`
  )
...
if (missing.length)
  console.log(
    `  ⚠ ${missing.length} pinned clip(s) missing from run: ${missing.join(', ')}`
  )
