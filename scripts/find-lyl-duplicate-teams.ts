...
;(async()=>{
  ...
  type T = { id: string; name: string; slug: string; matchCount: number; memberCount: number; stem: string }
  ...
  for (const team of lyl.teams as any[]) {
    ...
    if (!m) {
      ...
      arr.push({ id: team.id, name: team.name, slug: team.slug, matchCount: team.match_count ?? 0, memberCount: team.member_count ?? 0, stem: team.name.toLowerCase() })
      byAge.set('(other)', arr)
      continue
    }
    ...
    arr.push({ id: team.id, name: team.name, slug: team.slug, matchCount: team.match_count ?? 0, memberCount: team.member_count ?? 0, stem })
    byAge.set(age, arr)
  }
    ...
    for (let i = 0; i < teams.length; i++) {
      for (let j = i + 1; j < teams.length; j++) {
        if (namesAlias(teams[i].stem, teams[j].stem)) {
          collided.add(i); collided.add(j); collisions++
        }
      }
    }
    for (let i = 0; i < teams.length; i++) {
      ...
      console.log(`  ${mark}${t.name.padEnd(28)} slug=${t.slug.padEnd(30)} matches=${t.matchCount} members=${t.memberCount}`)
    }
    console.log()
  }
